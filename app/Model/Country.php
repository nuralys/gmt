...
public function customUploadImg($file = array()){
        ...
        $path = WWW_ROOT . 'img/country/' . $fileName;
        $path_th = WWW_ROOT . 'img/country/thumbs/' . $fileName;
        if(!move_uploaded_file($file['img']['tmp_name'], $path)){
            return false;
        }
        $this->resizeImg($path, $path_th, 260, 168, $ext);
    ...
    public function genNameFile($ext){
        ...
        if(is_file(WWW_ROOT . 'img/country/' . $name)){
